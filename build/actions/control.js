...
var require_dist_node11 = __commonJS({
  "node_modules/.pnpm/@octokit+request-error@5.0.0/node_modules/@octokit/request-error/dist-node/index.js"(exports2, module2) {
    "use strict";
...
// src/model.ts
var CommitRef = mod.string().brand("CommitRef");
...
var DeploymentConfig = mod.object({
  deployments: mod.record(StageName, mod.string().nullable())
});
var getDeploymentConfigAndSha = async (input = {}) => {
  try {
...
var getDeploymentConfig = async () => {
  const deploymentConfigRes = await getDeploymentConfigAndSha();
  if (deploymentConfigRes === void 0) {
    return void 0;
  }
  const [deploymentConfig] = deploymentConfigRes;
  return deploymentConfig;
};
var DeploymentConfigApi = {
  isDesiredCommitRefForStage: (config, commitRef, stage) => {
    return config.deployments[stage] === commitRef;
  },
  setDesiredCommitRefForStage: (config, commitRef, stage) => {
    return {
      ...config,
      deployments: {
        ...config.deployments,
...
// src/db/deployerDb.ts
var DeployStatus = mod.enum(["success", "failure"]).brand("DeploymentStatus");
var BuildStatus = mod.enum(["success", "failure"]).brand("BuildStatus");
var loadDb = async () => {
  return (await loadDbAndSha())?.[0];
};
var loadDbAndSha = async () => {
  try {
    ...
    if ("status" in error2 && error2.status === 404) {
      return void 0;
    }
    throw error2;
  }
};
var EMPTY_DB = { commitByRef: {}, stageByName: {}, commits: [] };
var changeDb = async (change) => {
  const [oldDb, sha] = await loadDbAndSha() ?? [EMPTY_DB, void 0];
  const newDb = immer_esm_default(change)(oldDb);
  if (!await checkBranchExists(BranchName.parse("deployer/db"))) {
    await createEmptyBranch("deployer/db");
  }
  const createOrUpdateFileContentsRes = await octokit.rest.repos.createOrUpdateFileContents({
    owner,
    repo,
    path: "deployer-db.json",
    message: "Save deployer-db.json",
    content: Buffer.from(JSON.stringify(newDb, void 0, 2) + "\n").toString(
      "base64"
    ),
    branch: "deployer/db",
    sha,
...
var createEmptyBranch = async (branchName) => {
...
var getCommitBuildStatus = async (commitRef, stageName) => {
  const db = await loadDb();
  return db?.commitByRef[commitRef]?.buildStatus;
};
var getDeployedCommitRef = async (stageName) => {
  const db = await loadDb();
...
var controlDeployment = async (input) => {
  ...
  if (await checkIfDeploymentIsInProgress({
...
async function checkIfCommitIsNotMarkedAsBuildPassed(commitRef, stageName) {
...
async function checkIfDeploymentIsInProgress(input) {
  ...
  const deployInProgressOrQueued = deployRunsRes.data.workflow_runs.some(
    (run2) => {
      ...
      return inProgressStatus.includes(run2.status);
    }
  );
...
var controlPullRequest = async (input) => {
  ...
  core3.info(`Current deployment config: "${JSON.stringify(deploymentConfig)}"`);
...
var createOrUpdatePr = async (input) => {
  core3.info(
  ...
  if (prNumber !== void 0) {
    core3.info(`PR already exists: #${prNumber}`);
    return;
  }
  core3.info(`Create PR`);
...
var createDeploymentConfigCommit = async (input) => {
  core3.info(
  ...
  if (deploymentConfigAndShaRes === void 0) {
  ...
  const createCommitResponse = await octokit.rest.repos.createOrUpdateFileContents({
    ...
    content: Buffer.from(
      JSON.stringify(newDeploymentConfig, void 0, 2) + "\n"
    ).toString("base64"),
...
var findPrNumber = async (input) => {
  const searchPrsRes = await octokit.rest.search.issuesAndPullRequests({
    q: `repo:${owner}/${repo}+head:${buildPrBranchName()}+base:${input.targetBranchName}+is:pr+is:open`
  });
...
var createOrResetBranch = async (input) => {
  core3.info(`Create or reset branch "${buildPrBranchName()}"`);
  ...
  if (await checkPrBranchExists()) {
    core3.info(`Branch "${buildPrBranchName()}" exists`);
    ...
    core3.info(`Branch "${buildPrBranchName()}" does not exist`);
...
var checkPrBranchExists = async () => {
  return await checkBranchExists(buildPrBranchName());
};
var buildPrBranchName = () => {
  return BranchName.parse("deployer/pr/dev");
};
var deletePr = async (input) => {
  core3.info("Delete PR");
  await deleteBranch();
  const prNumber = await findPrNumber({
    targetBranchName: input.branchName
  });
...
var getMostRecentDeployableCommitRef = async () => {
  const db = await loadDb();
  if (db === void 0) {
    return void 0;
  }
  ...
  for (const commitRef of commitRefs) {
    ...
    core3.info(`Most recent deployable commit ref: "${commitRef}"`);
    return commitRef;
  }
  return void 0;
};
...
var import_request_error = __toESM(require_dist_node11());
...
var DeployRun = mod.object({
  commitRef: CommitRef,
  stageName: StageName,
  deployStatus: DeployStatus,
  startedAt: mod.string(),
  finishedAt: mod.string()
}).brand("DeployRun");
var fetchFinishedDeployRuns = async (input) => {
  try {
    const deployRunsRes = await octokit.rest.actions.listWorkflowRuns({
      owner,
      repo,
    ...
    const deployRuns = deployRunsRes.data.workflow_runs.map((run2) => {
      const deployStatus = DeployStatus.safeParse(run2.conclusion);
...
var controlCommits = async (input) => {
  ...
  try {
    ...
    const deployRunsSortedByFinishedAt = deployRuns.sort((a2, b2) => {
      return new Date(b2.finishedAt).getTime() - new Date(a2.finishedAt).getTime();
    });
    const deployRunsSortedByFinishedAtAndGroupedByStageName = groupBy_default(
      deployRunsSortedByFinishedAt,
      (deployRun) => {
        return deployRun.stageName;
      }
    );
    const deployRunsByCommitRef = /* @__PURE__ */ new Map();
    ...
    await changeDb((oldDb) => {
      ...
      for (const [rawStageName, deployRuns2] of Object.entries(
        deployRunsSortedByFinishedAtAndGroupedByStageName
      )) {
        const latestDeployRun = deployRuns2[0];
        if (latestDeployRun === void 0) {
          continue;
        }
...
var fetchBuildRuns = async (input) => {
  try {
    ...
    const buildRuns = buildRunsRes.data.workflow_runs.map((run2) => {
      const buildStatusRes = BuildStatus.safeParse(run2.conclusion);
      if (!buildStatusRes.success) {
        return;
      }
      const buildRun = BuildRun.parse({
        commitRef: run2.head_sha,
...
var control = async (input) => {
  ...
  try {
    ...
    core6.warning(`Error while controlling commits: ${error2}`);
  }
  try {
    await controlPullRequest(input);
  } catch (error2) {
    console.error(error2);
    core6.warning(`Error while controlling pull request: ${error2}`);
  }
  try {
