...
async function checkIfDeploymentIsInProgress(commitRef, stageName) {
...
var fetchFinishedDeployRuns = async () => {
  try {
    const deployRunsRes = await octokit.rest.actions.listWorkflowRuns({
      owner,
      repo,
...
var control = async (input) => {
  ...
  try {
    ...
    core6.warning(`Error while controlling commits: ${error}`);
  }
  try {
    await controlPullRequest(input);
  } catch (error) {
    core6.warning(`Error while controlling pull request: ${error}`);
  }
  try {
    await controlDeployment(input);
  } catch (error) {
    core6.warning(`Error while controlling deployment: ${error}`);
  }
};
