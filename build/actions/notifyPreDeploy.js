...
// src/actions/notifyPreDeploy.ts
var notifyPreDeploy_exports = {};
__export(notifyPreDeploy_exports, {
  run: () => run
});
module.exports = __toCommonJS(notifyPreDeploy_exports);
...
var run = async () => {
  ...
  const commitRef = CommitRef.parse(github.context.sha);
  core.info(`Notify pre-deploy with outcome "${outcome}"`);
};
run();
